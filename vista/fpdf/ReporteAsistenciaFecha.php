...
if (!empty($_GET["txtfechainicio"]) and !empty($_GET["txtfechafinal"]) and !empty($_GET["txtempleado"])) {
   ...
   class PDF extends FPDF
   {
      ...
      function Header()
      {
         ...
         $this->Cell(100, 10, utf8_decode("REPORTE DE ASISTENCIAS POR FECHAS $fechaInicio - $fechaFinal"), 0, 1, 'C', 0);
         $this->Ln(7);
   ...
   /*consulta guardada "SELECT
      ...
      IF(asistencia.salida < asistencia.entrada,
          TIME_FORMAT(ADDTIME(TIMEDIFF(asistencia.salida, asistencia.entrada), '24:00:00'), '%H:%i:%s'),
          TIME_FORMAT(TIMEDIFF(asistencia.salida, asistencia.entrada), '%H:%i:%s')
      ) AS 'totalHR',
      empleado.nombre,
      empleado.apellido,
      empleado.dni,
      ...
      where entrada BETWEEN '$fechaInicio 00:00:00' and '$fechaFinal 23:59:59' order by id_empleado asc"
      */
   if ($empleado == "todos") {
      ...
      $totalHoras = $conexion->query(" SELECT
      asistencia.id_empleado,
      empleado.nombre,
      empleado.apellido,
      empleado.dni,
      cargo.nombre AS 'cargo',
      SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(asistencia.salida, asistencia.entrada)))) AS horas
  FROM
      asistencia
      INNER JOIN empleado ON asistencia.id_empleado = empleado.id_empleado
      INNER JOIN cargo ON empleado.cargo = cargo.id_cargo
  WHERE
      asistencia.entrada BETWEEN '$fechaInicio 00:00:00' AND '$fechaFinal 23:59:59'
  GROUP BY
      asistencia.id_empleado,
      empleado.nombre,
      empleado.apellido,
  ...
  ORDER BY
   ...
   ");
     ...
     INNER JOIN empleado ON asistencia.id_empleado = empleado.id_empleado
     INNER JOIN cargo ON empleado.cargo = cargo.id_cargo
     where asistencia.id_empleado=$empleado and entrada BETWEEN '$fechaInicio 00:00:00' and '$fechaFinal 23:59:59' order by id_asistencia asc ");
      ...
      INNER JOIN cargo ON empleado.cargo = cargo.id_cargo
      WHERE
      asistencia.id_empleado=$empleado and asistencia.entrada BETWEEN '$fechaInicio 00:00:00' AND '$fechaFinal 23:59:59'
      GROUP BY
      asistencia.id_empleado,
      empleado.nombre,
      empleado.apellido,
...
");
   ...
   //agregar tabla que muestre el nombre del empleado y el total de horas trabajadas
   //agregar un salto mas abajo
   $pdf->Ln(10);
   $pdf->SetFillColor(125, 173, 221);
   $pdf->Cell(100, 10, utf8_decode("Nombre del empleado"), 1, 0, 'C', 1);
   $pdf->Cell(50, 10, utf8_decode("Total de Horas"), 1, 1, 'C', 1);

   while ($datos_reporte = $totalHoras->fetch_object()) {
      $pdf->Cell(100, 10, utf8_decode($datos_reporte->nombre . " " . $datos_reporte->apellido), 1, 0, 'C', 0);
      $pdf->Cell(50, 10, utf8_decode($datos_reporte->horas), 1, 1, 'C', 0);
   }
