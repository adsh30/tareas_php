...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Image('logo.png', 20, 5, 40); //logo de la empresa,moverDerecha,moverAbajo,tamañoIMG
      ...
      $this->Cell(105, 10, utf8_decode('DESCRIPCION'), 1, 1, 'C', 1);
   }
...
$consulta_reporte_usuario = $conn->query(" SELECT * from department_list ");

while ($datos_reporte = $consulta_reporte_usuario->fetch_object()) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(25, 10, utf8_decode($i), 1, 0, 'C', 0);
   $pdf->Cell(60, 10, utf8_decode($datos_reporte->department), 1, 0, 'C', 0);
   $pdf->Cell(105, 10, utf8_decode($datos_reporte->description), 1, 1, 'C', 0);
}
